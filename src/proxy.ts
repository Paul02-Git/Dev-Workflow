...
export const config = {
  matcher: [
    ...
    // portal/intake pages (deliberately unauthenticated — gated by their
    // own unguessable tokens instead), the portal's own polling API route
    // (api/portal/... — same public/token-gated reasoning, needs its own
    // entry since it doesn't start with "portal"), static assets, and
    ...
    // or /favicon.ico).
    "/((?!login|handoff|portal|intake|api/portal|_next/static|_next/image|icon|apple-icon|.*\\.(?:ico|png|jpg|jpeg|svg|gif|webp)$).*)",
  ],
};
