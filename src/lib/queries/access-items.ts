 ...
 */
export async function listAccessItems(projectId: string) {
  return db
    .select({
      id: accessItems.id,
    ...
    .from(accessItems)
    .where(eq(accessItems.projectId, projectId))
...
export async function updateAccessItem(
  ...
  if (status) {
    ...
    if (status === "GRANTED" || status === "VERIFIED") {
      const [current] = await db.select({ grantedAt: accessItems.grantedAt }).from(accessItems).where(eq(accessItems.id, id));
      if (!current?.grantedAt) patch.grantedAt = new Date();
    }
  }

  const [item] = await db.update(accessItems).set(patch).where(eq(accessItems.id, id)).returning();
  return item;
}

/** Removes stored username/password entirely, without deleting the access item itself. */
export async function clearAccessItemCredentials(id: string): Promise<void> {
  await db.update(accessItems).set({ username: null, passwordEncrypted: null, updatedAt: new Date() }).where(eq(accessItems.id, id));
}
 ...
 */
export async function setAccessItemCredentials(id: string, input: { username?: string; password?: string }) {
  const [row] = await db.select({ projectId: accessItems.projectId }).from(accessItems).where(eq(accessItems.id, id));
...
export async function revealAccessItemPassword(id: string): Promise<string | null> {
  const [row] = await db
